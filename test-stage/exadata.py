...
def _part_file(root, ym, plugin):
    """Physical parquet file path for one month/plugin slice."""
    return os.path.join(_partition_dir(root, ym, plugin), "part-000.parquet")


def _safe_columns(parquet_path, wanted):
    """Keep only columns that physically exist in the parquet file."""
    pf = pq.ParquetFile(parquet_path)
    have = set(pf.schema_arrow.names)
    return [c for c in wanted if c in have]


def _read_physical_columns(parquet_path, wanted):
    """Read selected flat columns from one physical Parquet file.

    This deliberately avoids pyarrow.dataset Hive schema merging. It also avoids
    nested/list columns such as job `nodes`, which can trigger errors such as
    'repetition level histogram size mismatch' in some M100 files.
    """
    cols = _safe_columns(parquet_path, wanted)
    if not cols:
        return pd.DataFrame()
    try:
        table = pq.ParquetFile(parquet_path).read(columns=cols)
        return table.to_pandas()
    except Exception as first_err:
        # Fallback: read columns one by one and skip any corrupted/problematic column.
        frames = []
        bad = []
        for c in cols:
            try:
                t = pq.ParquetFile(parquet_path).read(columns=[c])
                frames.append(t.to_pandas())
            except Exception as ex:
                bad.append(f"{c}: {ex}")
        if not frames:
            raise RuntimeError(
                f"Could not read selected columns from {parquet_path}. "
                f"First error: {first_err}. Column errors: {bad}"
            )
        df = pd.concat(frames, axis=1)
        # Do not fail the whole run for optional columns.
        return df
...
def load_jobs(root, ym, t0, t1):
    ...
    Physical-file reader version. Avoids pyarrow.dataset and avoids nested
    columns, which fixes both:
      - Hive schema merge errors on year_month/plugin
      - repetition level histogram size mismatch from nested/list columns
    """
    _require_pyarrow()
    root = _resolve_root(root)
    t0, t1 = _to_utc(t0), _to_utc(t1)
    pf = _part_file(root, ym, JOB_PLUGIN)
    if not os.path.exists(pf):
        return pd.DataFrame()

    # Keep only flat columns needed for modeling. Do NOT read `nodes`.
    wanted = [
        "job_id", "submit_time", "start_time", "end_time",
        "job_state", "partition", "qos", "user_id",
        "num_nodes", "num_cpus", "num_tasks", "metric",
    ]
    df = _read_physical_columns(pf, wanted)
    if df.empty:
        return df
...
def load_measured(root, ym, t0, t1, source, metric=None):
    """Cluster-total measured power (kW) on native cadence for [t0, t1).

    Physical-file reader version. Reads only timestamp, metric, value_numeric,
    so it avoids Hive partition merging and nested-column decode errors.
    """
    _require_pyarrow()
    ...
    root = _resolve_root(root)
    t0, t1 = _to_utc(t0), _to_utc(t1)
    pf = _part_file(root, ym, spec["plugin"])
    if not os.path.exists(pf):
        return pd.DataFrame(columns=["timestamp", "kw"])

    df = _read_physical_columns(pf, ["timestamp", "metric", "value_numeric"])
    if df.empty:
        return pd.DataFrame(columns=["timestamp", "kw"])

    df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True, errors="coerce")
    df["value_numeric"] = pd.to_numeric(df["value_numeric"], errors="coerce")
    df = df.dropna(subset=["timestamp", "metric", "value_numeric"])
    df = df[(df["timestamp"] >= t0) & (df["timestamp"] < t1)]
    return _aggregate_measured(df, spec, metric)


def list_metrics(root, ym, plugin, limit=50):
    """Distinct metric names and counts from one physical plugin file."""
    _require_pyarrow()
    root = _resolve_root(root)
    pf = _part_file(root, ym, plugin)
    if not os.path.exists(pf):
        return []
    df = _read_physical_columns(pf, ["metric"])
    if df.empty or "metric" not in df.columns:
        return []
    s = df["metric"].astype(str).str.strip()
